...
# --- KONFIGURASI HALAMAN ---
st.set_page_config(page_title="Aplikasi Retase & Muatan", layout="wide", page_icon="lembu.png")
...
# --- 1. PDF ---
def generate_pdf(conn, lokasi_id, nama_lokasi, start_date_g, end_date_g, judul, material, satuan_lokasi, is_landscape, include_chart):
    buffer = io.BytesIO()
    ...
    if is_landscape:
        ...
        needed_height = 350 + (max_rows * 20)
        page_height = max(landscape(A4)[1], needed_height) 
        page_format = (page_width, page_height)
    else:
        page_format = portrait(A4)

    doc = SimpleDocTemplate(buffer, pagesize=page_format, rightMargin=15, leftMargin=15, topMargin=15, bottomMargin=15)
    elements = []
    ...
    for idx, (s_date, e_date) in enumerate(split_date_range_by_month(start_date_g, end_date_g)):
        ...
        lbl_vol = "KUBIKASI" if satuan_lokasi == 'm³' else "TONASE"
        lbl_tot = "TOTAL KUBIKASI" if satuan_lokasi == 'm³' else "TOTAL TONASE"

        t_data = [['NO', 'HARI', 'TANGGAL', 'RETASE', 'ISI/1 RET', lbl_vol]]
        tot_ret = 0; tot_kub = 0.0
        if not rekap.empty:
            for i, row in rekap.iterrows():
                t_data.append([str(i+1), row['hari'], row['tanggal'].strftime('%d/%m/%Y'), f"{int(row['jumlah_retase'])}", f"{row['isi_per_ret']:.1f} {satuan_lokasi}", f"{row['kubikasi']:.1f} {satuan_lokasi}"])
                tot_ret += row['jumlah_retase']; tot_kub += row['kubikasi']
        else: t_data.append(['-', '-', '-', '-', '-', '-'])

        t_data.extend([['', '', '', 'RINCIAN', '', ''], ['', '', '', 'TOTAL RETASE', f"{int(tot_ret)} Ret", ''], ['', '', '', lbl_tot, f"{tot_kub:,.2f} {satuan_lokasi}", '']])
    ...
    if include_chart:
        ...
        if img_m_buf: 
            img_rl_m = RLImage(img_m_buf, width=450 if is_landscape else 400, height=200)
            img_rl_m.hAlign = 'CENTER'
            elements.append(img_rl_m)
            elements.append(Spacer(1, 15))
        
    tm_data = [['NO', 'BULAN', lbl_vol]]
    for i, r in df_m.iterrows(): tm_data.append([str(i+1), r['Bulan'], f"{r['Kubikasi']:,.2f} {satuan_lokasi}"])
    tm_data.append(['', 'TOTAL', f"{tot_all_kb:,.2f} {satuan_lokasi}"])
...
# --- 2. EXCEL ---
def generate_excel(conn, lokasi_id, nama_lokasi, start_date_g, end_date_g, judul, material, satuan_lokasi, include_chart):
    ...
    for s_date, e_date in split_date_range_by_month(start_date_g, end_date_g):
        ...
        lbl_vol = "KUBIKASI" if satuan_lokasi == 'm³' else "TONASE"
        lbl_tot = "JUMLAH KUBIKASI" if satuan_lokasi == 'm³' else "JUMLAH TONASE"
        ...
        if include_chart:
            ...
            ws.merge_cells('A1:F1'); ws['A1'] = f"{judul} {material}"; ws['A1'].font = Font(bold=True, size=14); ws['A1'].alignment = Alignment(horizontal='center')
            ws.merge_cells('A2:F2'); ws['A2'] = nama_lokasi; ws['A2'].font = Font(bold=True, size=12); ws['A2'].alignment = Alignment(horizontal='center')
            ws.merge_cells('A3:F3'); ws['A3'] = f"PERIODE {get_bulan_indonesia(s_date.month).upper()} {s_date.year}"; ws['A3'].alignment = Alignment(horizontal='center')

        r = 5
        headers = ['NO', 'HARI', 'TANGGAL', 'JUMLAH RETASE', 'ISI/1 RET', lbl_vol]
        ...
        if not rekap.empty:
            for i, row in rekap.iterrows():
                vals = [i+1, row['hari'], row['tanggal'].strftime('%d/%m/%Y'), int(row['jumlah_retase']), f"{row['isi_per_ret']} {satuan_lokasi}", f"{row['kubikasi']} {satuan_lokasi}"]
        ...
        ws.cell(r,4,lbl_tot).border=thin; ws.cell(r,4).font=Font(bold=True); ws.merge_cells(start_row=r, start_column=5, end_row=r, end_column=6); c=ws.cell(r,5,f"{tot_kub:,.2f} {satuan_lokasi}"); c.border=thin; c.font=Font(bold=True); c.alignment=Alignment(horizontal='center'); ws.cell(r,6).border=thin
    ...
    for i, h in enumerate(['NO', 'BULAN', lbl_vol]): c=ws2.cell(r2, i+1, h); c.border=thin; c.fill=PatternFill("solid", fgColor="2F5496"); c.font=Font(color="FFFFFF", bold=True); c.alignment=Alignment(horizontal='center')
    r2 += 1
    for i, row in df_m.iterrows():
        vals = [i+1, row['Bulan'], f"{row['Kubikasi']:,.2f} {satuan_lokasi}"]
    ...
    ws2.cell(r2, 3, f"{tot_all_kb:,.2f} {satuan_lokasi}").font=Font(bold=True); ws2.cell(r2, 3).border=thin; ws2.cell(r2, 3).fill=PatternFill("solid", fgColor="FFD966")
...
# --- 3. WORD ---
def generate_docx(conn, lokasi_id, nama_lokasi, start_date_g, end_date_g, judul, material, satuan_lokasi, is_landscape, include_chart):
    doc = Document()
    if is_landscape:
    ...
    for idx, (s_date, e_date) in enumerate(split_date_range_by_month(start_date_g, end_date_g)):
        ...
        if include_chart:
            ...
            target_container = layout_table.cell(0, 0)
        else:
            target_container = doc

        t = target_container.add_table(rows=1, cols=6); t.style = 'Table Grid'
        if not include_chart: t.alignment = WD_ALIGN_PARAGRAPH.CENTER

        hdr = t.rows[0].cells
        for i, text in enumerate(['NO', 'HARI', 'TANGGAL', 'JUMLAH RETASE', 'ISI/1 RET', lbl_vol]):
        ...
        if not rekap.empty:
            for i, row in rekap.iterrows():
                r = t.add_row().cells
                r[0].text = str(i+1); r[1].text = row['hari']; r[2].text = row['tanggal'].strftime('%d/%m/%Y'); r[3].text = str(int(row['jumlah_retase'])); r[4].text = f"{row['isi_per_ret']} {satuan_lokasi}"; r[5].text = f"{row['kubikasi']} {satuan_lokasi}"
        ...
        r3 = t.add_row().cells; r3[3].text = lbl_tot; r3[4].text = f"{tot_kub:,.2f} {satuan_lokasi}"; r3[4].merge(r3[5]); r3[3].paragraphs[0].runs[0].font.bold=True; r3[4].paragraphs[0].runs[0].font.bold=True; r3[3].paragraphs[0].runs[0].font.size=Pt(8); r3[4].paragraphs[0].runs[0].font.size=Pt(8); r3[4].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    for i, text in enumerate(['NO', 'BULAN', lbl_vol]):
    ...
    for i, row in df_m.iterrows():
        r = t_m.add_row().cells; r[0].text = str(i+1); r[1].text = row['Bulan']; r[2].text = f"{row['Kubikasi']:,.2f} {satuan_lokasi}"
        for c in r: c.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_t = t_m.add_row().cells; r_t[1].text = "TOTAL"; r_t[2].text = f"{tot_all_kb:,.2f} {satuan_lokasi}"; r_t[1].paragraphs[0].runs[0].font.bold=True; r_t[2].paragraphs[0].runs[0].font.bold=True; r_t[2].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
...
def main():
    ...
    try: 
        ...
        # Tambahkan kolom satuan_proyek di lokasi_proyek
        cursor.execute("""CREATE TABLE IF NOT EXISTS lokasi_proyek (id INT AUTO_INCREMENT PRIMARY KEY, nama_tempat VARCHAR(255), kunci_lokasi VARCHAR(255), judul_laporan VARCHAR(255) DEFAULT 'JUMLAH RETASE DAN KUBIKASI OB', jenis_material VARCHAR(255) DEFAULT 'TANAH', satuan_proyek VARCHAR(20) DEFAULT 'm³')""")
        
        # Coba alter table jika db lama
        try: cursor.execute("SELECT satuan_proyek FROM lokasi_proyek LIMIT 1"); cursor.fetchall()
        except: cursor.execute("ALTER TABLE lokasi_proyek ADD COLUMN satuan_proyek VARCHAR(20) DEFAULT 'm³'"); conn.commit()
    ...
    if st.session_state.active_project_id is None:
        ...
        with col_right:
            with st.container(border=True):
                ...
                # FITUR BARU: PILIH SATUAN DI SINI
                satuan_proyek_baru = st.selectbox("Satuan Pengukuran Proyek Ini:", ["Kubikasi (m³)", "Tonase (Ton)"])
                sat_val_baru = "m³" if "m³" in satuan_proyek_baru else "Ton"
                ...
                if st.button("Simpan Lokasi Baru", use_container_width=True):
                    if new_name and new_pass: cursor.execute("INSERT INTO lokasi_proyek (nama_tempat, kunci_lokasi, satuan_proyek) VALUES (%s, %s, %s)", (new_name, new_pass, sat_val_baru)); conn.commit(); st.success("Dibuat!"); st.rerun()
    ...
    cursor.execute("SELECT judul_laporan, jenis_material, satuan_proyek FROM lokasi_proyek WHERE id=%s", (lokasi_id,))
    ...
    satuan_proyek_aktif = res_set[2] if res_set[2] else "m³"

    # Penamaan Label Dinamis
    lbl_vol = "KUBIKASI" if satuan_proyek_aktif == 'm³' else "TONASE"
    lbl_tot_vol = "TOTAL KUBIKASI" if satuan_proyek_aktif == 'm³' else "TOTAL TONASE"
    ...
    with t1:
        ...
        with st.form("form_input"):
            ...
            with c3: ir = st.number_input(f"Isi / 1 Ret ({satuan_proyek_aktif})", min_value=0.1, step=0.1, value=5.0)
            ...
            if st.session_state.edit_id:
                ...
                if res:
                    with st.form("edit_form"):
                        ...
                        with c3: e_ir = st.number_input(f"Isi / 1 Ret ({satuan_proyek_aktif})", value=float(res[2]))
            ...
            if not df_all.empty:
                ...
                with st.container(height=400, border=True):
                    for _, row in df_view.head(50).iterrows():
                        ...
                        cb.write(f"**{row['jumlah_retase']} Ret** x {row['isi_per_ret']} {satuan_proyek_aktif}")
                        cc.write(f"**{row['kubikasi']:,.1f} {satuan_proyek_aktif}**")
                        with cd:
                            b1, b2 = st.columns(2)
    ...
    with t2:
        ...
        if not df_rep.empty:
            tot_val = df_rep['kubikasi'].sum()
            tot_ret = df_rep['jumlah_retase'].sum()
            st.markdown(f"""<div style="background-color:#D9E1F2;padding:15px;border-radius:10px;border:1px solid #2F5496;text-align:center;margin-bottom:20px;"><h2 style="color:#2F5496;margin:0;">{lbl_tot_vol}: {tot_val:,.2f} {satuan_proyek_aktif}</h2><span style="color:#333;font-weight:bold;">Total Retase: {tot_ret:,.0f} Trip</span></div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div style="background-color:#D9E1F2;padding:15px;border-radius:10px;border:1px solid #2F5496;text-align:center;margin-bottom:20px;"><h2 style="color:#2F5496;margin:0;">TOTAL DATA: 0</h2></div>""", unsafe_allow_html=True)
        ...
        with st.expander("🛠️ PENGATURAN NAMA LAPORAN & PROYEK", expanded=False):
            with st.form("form_setting"):
                ...
                s2 = st.text_input("Jenis Material (Ketik bebas):", value=jenis_mat_aktif)
        ...
        st.divider()
        st.subheader("📋 Detail Data Retase & Muatan")
        if not df_rep.empty:
            df_display = df_rep[['tanggal', 'jumlah_retase', 'isi_per_ret', 'kubikasi', 'keterangan']].copy()
            df_display.rename(columns={'tanggal':'Tanggal', 'jumlah_retase':'Jumlah Retase', 'isi_per_ret':f'Isi / 1 Ret ({satuan_proyek_aktif})', 'kubikasi':f'Total {lbl_vol} ({satuan_proyek_aktif})', 'keterangan':'Keterangan'}, inplace=True)
    ...
    with t3:
        ...
        include_chart = st.checkbox("Sertakan Grafik / Diagram dalam hasil Export", value=True)
            ...
            with c1:
                if st.button("📕 Download PDF", use_container_width=True): 
                    with st.spinner("Membuat PDF..."):
                        pdf = generate_pdf(conn, lokasi_id, nama_proyek, start_date_exp, end_date_exp, judul_lap_aktif, jenis_mat_aktif, satuan_proyek_aktif, is_landscape, include_chart)
                        st.download_button("⬇️ Simpan PDF", pdf, f"Retase_{nama_proyek}.pdf", "application/pdf")
            with c2:
                if st.button("📗 Download Excel", use_container_width=True): 
                    with st.spinner("Membuat Excel..."):
                        xl = generate_excel(conn, lokasi_id, nama_proyek, start_date_exp, end_date_exp, judul_lap_aktif, jenis_mat_aktif, satuan_proyek_aktif, include_chart)
            ...
            with c3:
                if st.button("📘 Download Word", use_container_width=True): 
                    with st.spinner("Membuat Word..."):
                        doc = generate_docx(conn, lokasi_id, nama_proyek, start_date_exp, end_date_exp, judul_lap_aktif, jenis_mat_aktif, satuan_proyek_aktif, is_landscape, include_chart)
